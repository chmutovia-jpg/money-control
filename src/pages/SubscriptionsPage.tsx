...
const blank: SubscriptionForm = { name: "", amount: "", period: "monthly", nextPaymentDate: todayISO(), category: "развлечения", isActive: true };
      ...
      <div className="grid gap-5 lg:grid-cols-[380px_1fr]">
        ...
        <div className="space-y-5">
          <Card className="bg-gradient-to-br from-slate-800/70 via-slate-900/80 to-blue-950/70 text-white">
            ...
            <p className="mt-2 text-4xl font-bold">{formatCurrency(getMonthlySubscriptionsTotal(subscriptions))}</p>
            ...
            {subscriptions.length ? <div className="space-y-3">{subscriptions.map((item) => {
              ...
              return <div key={item.id} className={`rounded-3xl border p-4 ${near ? "border-amber-200 bg-amber-50" : "border-slate-100 bg-slate-50"}`}>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div><p className="font-bold text-ink">{item.name}</p><p className="mt-1 text-sm text-muted">{item.category} • {formatDate(item.nextPaymentDate)} • {item.period === "monthly" ? "ежемесячно" : "ежегодно"}</p><p className="mt-2 text-lg font-bold text-sky-600">{formatCurrency(monthlySubscriptionAmount(item))} / мес.</p></div>
                  <div className="flex gap-2"><button className={ghostButtonClass} type="button" onClick={() => { setEditingId(item.id); setForm({ ...item, amount: String(item.amount) }); }}><Edit3 size={17} />Изм.</button><button className="rounded-2xl bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-600" type="button" onClick={() => onDelete(item.id)}><Trash2 size={17} /></button></div>
                </div>
              </div>;
